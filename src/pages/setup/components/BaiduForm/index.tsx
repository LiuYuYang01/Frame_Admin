...
import { updateEnvConfigDataAPI, type BaiduStatisEnvValue } from '@/api/config';
import type { SetupFormProps } from '../types';

export function BaiduForm({ row, onSaved }: SetupFormProps) {
  const [form] = Form.useForm<BaiduStatisEnvValue>();
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    const value = row?.value as unknown as BaiduStatisEnvValue | undefined;
    form.setFieldsValue({
      site_id: value?.site_id ?? 0,
